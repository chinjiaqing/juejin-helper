...
const articleComment = async task => {
    ...
    const commentIds = []
    const defaultComments = [`感谢，学习了，受益颇多`, '竟然还能这样，妙啊！', '学到了', '听君一席话，如同听君一席话', '我虽然看不懂，但我大受震撼']
    ...
    for (let i = 0; i < times; i++) {
        ...
        const comment = await API.articleCommentAdd(article['article_id'], words)
        // 删除评论
        // await API.articleCommentRemove(comment['comment_id'])
        commentIds.push(comment['comment_id'])
    }
    // 将评论id 缓存下来，第二天移除
    logger.set('articles.comments', commentIds).save()
